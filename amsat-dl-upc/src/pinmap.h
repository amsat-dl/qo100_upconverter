#ifndef __PINMAP_H__
#define __PINMAP_H__

#include <driver/gpio.h>

//port A
GPIO_DEFINE_PIN_H(PIN_ADF_LOCK)
GPIO_DEFINE_PIN_H(PIN_ADF_CE)
GPIO_DEFINE_PIN_H(PIN_ADF_LE)
GPIO_DEFINE_PIN_H(PIN_SIO_DATA)
GPIO_DEFINE_PIN_H(PIN_SIO_CLK)
GPIO_DEFINE_PIN_H(PIN_ADF4351_TMP)
GPIO_DEFINE_PIN_H(PIN_ADF_FREQ_SEL)
GPIO_DEFINE_PIN_H(PIN_PTT_INPUT)

//port B
GPIO_DEFINE_PIN_H(PIN_TEMP)
GPIO_DEFINE_PIN_H(PIN_RF_V) //vorlauf
GPIO_DEFINE_PIN_H(PIN_RF_R) //rücklauf
GPIO_DEFINE_PIN_H(PIN_VOLTAGE)
GPIO_DEFINE_PIN_H(PIN_FSW0)
GPIO_DEFINE_PIN_H(PIN_FSW1)

//port C
GPIO_DEFINE_PIN_H(PIN_LED_CPU_OK)
GPIO_DEFINE_PIN_H(PIN_LED_ADF_LOCK)
GPIO_DEFINE_PIN_H(PIN_LED_PTT_ACTIVE)
GPIO_DEFINE_PIN_H(PIN_ALARM_OUTPUT)
GPIO_DEFINE_PIN_H(PIN_PA_ENABLE)
GPIO_DEFINE_PIN_H(PIN_USART_TX)
GPIO_DEFINE_PIN_H(PIN_USART_RX)
        
#endif //__PINMAP_H__
